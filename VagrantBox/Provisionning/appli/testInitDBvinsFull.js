...
try {
 ...
 db.transaction(function (tx) {
    tx.executeSql('DROP TABLE IF EXISTS RECOLTE',null,null,function (tx, err) {
  		document.getElementById('deletetables').innerHTML = 'Erreur suppression RECOLTES'; } );
    tx.executeSql('DROP TABLE IF EXISTS VINS',null,null,function (tx, err) {
  		document.getElementById('deletetables').innerHTML = 'Erreur suppression VINS';});
    tx.executeSql('DROP TABLE IF EXISTS PRODUCTEURS',null,null,function (tx, err) {
  		document.getElementById('deletetables').innerHTML = 'Erreur suppression PRODUCTEURS';});
	});
 	document.getElementById('deletetables').innerHTML = 'Tables supprimées';
db.transaction(function (tx) {
	 tx.executeSql("CREATE TABLE PRODUCTEURS (num integer c, nom char(40) not NULL, prenom char(40), "
		+ "region char(40) CONSTRAINT Cregion CHECK (region in ('Bourgogne', 'Beaujolais', 'Alsace', 'Jura', "
		+ "'Corse','Rhone','Jura','Pyrenees','Provence','Languedoc', 'Sud Ouest','Savoie')), "
		+ "PRIMARY KEY (num))",null,null,function (tx, err) {
  		document.getElementById('createtables').innerHTML = 'Erreur creation PRODUCTEURS'; } );
    tx.executeSql('CREATE TABLE VINS (num integer, cru char (40) NOT NULL, annee integer CONSTRAINT Cannee CHECK (annee between 1970 and 2020),'
		+ 'degre numeric(4,2) CONSTRAINT Cdegre CHECK (degre between 9.0 and 15.0), PRIMARY KEY (num))',
			null,null,function (tx, err) {
  				document.getElementById('createtables').innerHTML = 'Erreur creation VINS'; } );
	 tx.executeSql('CREATE TABLE RECOLTES (nprod integer references PRODUCTEURS(num), nvin integer references VINS(num) on delete cascade, quantite integer, PRIMARY KEY (nprod, nvin))',
			null,null,function (tx, err) {
  				document.getElementById('createtables').innerHTML = 'Erreur creation RECOLTES'; 
  				} );
  });
  document.getElementById('createtables').innerHTML = 'Creation Tables OK'; 
   db.transaction(function (tx) {
   	tx.executeSql(
   ...
   + "(129, 'Barrere', 'Alfred', 'Sud Ouest')" ,null,function (tx, result) {
  		document.getElementById('insertproducteurs').innerHTML = 'Ajout PRODUCTEURS OK';}
  		,function (tx, err) {
  		document.getElementById('insertproducteurs').innerHTML = 'Erreur ajout PRODUCTEURS';});
