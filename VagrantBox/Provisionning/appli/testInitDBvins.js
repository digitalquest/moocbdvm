...
var recoltes = [
 ...
 db.transaction(function (tx) {
    tx.executeSql('DROP TABLE IF EXISTS RECOLTES',null,null,function (t, err) {
  		document.getElementById('deletetables').innerHTML = 'Erreur suppression RECOLTES ' + err.message; } );
    tx.executeSql('DROP TABLE IF EXISTS VINS',null,null,function (t, err) {
  		document.getElementById('deletetables').innerHTML = 'Erreur suppression VINS ' + err.message;});
    tx.executeSql('DROP TABLE IF EXISTS PRODUCTEURS',null,null,function (t, err) {
  		document.getElementById('deletetables').innerHTML = 'Erreur suppression PRODUCTEURS ' + err.message;});
	});
...
db.transaction(function (tx) {
	 tx.executeSql("CREATE TABLE PRODUCTEURS (num integer c, nom char(40) not NULL, prenom char(40), "
		...
		+ "PRIMARY KEY (num))",null,null,function (t, err) {
  		document.getElementById('createtables').innerHTML = 'Erreur creation PRODUCTEURS ' + err.message; } );
    tx.executeSql('CREATE TABLE VINS (num integer, cru char (40) NOT NULL, annee integer CONSTRAINT Cannee CHECK (annee between 1970 and 2020),'
		+ 'degre numeric(4,2) CONSTRAINT Cdegre CHECK (degre between 9.0 and 15.0), PRIMARY KEY (num))',
			null,null,function (t, err) {
  				document.getElementById('createtables').innerHTML = 'Erreur creation VINS ' + err.message; } );
  ...
  document.getElementById('createtables').innerHTML = 'Creation Tables OK'; 
  	...
  	db.transaction(
	  		function (tx) { 
  	  		  ...
  	  		  for(ind = 0; ind < producteurs.length; ind++) {
     			tx.executeSql(
 	...
 	document.getElementById('insertproducteurs').innerHTML = 'Insertion des ' + producteurs.length + ' producteurs.';
	db.transaction(
  		function (tx) { 
  		var ind;
			for(ind = 0; ind < vins.length; ind++) {     			
				tx.executeSql(
	...
	document.getElementById('insertvins').innerHTML = "Insertion des " + vins.length + " vins";

	db.transaction(
  	  		function (tx) { 
  	  			var ind;
  	  			for(ind = 0; ind < recoltes.length; ind++) {
     			tx.executeSql(
	...
	document.getElementById('insertrecoltes').innerHTML = "Insertion des  " + recoltes.length + " récoltes";
